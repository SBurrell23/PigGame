...
onMounted(() => {
  if (props.autoOpen && detailsRef.value) {
    detailsRef.value.open = true
  }
})

watch(() => props.autoOpen, (val) => {
  if (val && detailsRef.value) {
    detailsRef.value.open = true
  }
})
</script>
...
<template>
  <details ref="detailsRef" class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-600 overflow-hidden transition-colors duration-300">
